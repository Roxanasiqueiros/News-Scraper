$("#scrape-data").on("click", function() {
    ...
    newDiv.html(
      $(`<h2 id="processing-instructions">Processing your request...</h2>`)
    );
    ...
    $.ajax({
      ...
      if (data.length) {
        for (let i = 0; i < data.length; i++) {
          ...
          newDiv.append(
            $(
              `<h5><button class="save-article btn btn-primary" data="${data[i]._id}">Save Article </button></h5>`
            )
          );
        ...
        let newDiv = $(`<div id="empty-article-display">`);
        newDiv.append(
          $(`<h2 id="instructions">Press on the button Scrape New Articles</h2>`)
        );
