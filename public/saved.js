    ...
    $.ajax({
      ...
      if (data.length) {
        for (let i = 0; i < data.length; i++) {
          let newDiv = $(
            `<div class="saved-article-container" data="${data[i]._id}" id="article">`
          );
        ...
        newDiv.append(
          $(`<h2 id="instructions">Please Save Articles On The Homepage!</h2>`)
        );
    ...
    $.ajax({
      ...
      if (data.note.length) {
        for (let i = 0; i < data.note.length; i++) {
          notesDiv.append(
            `<h5 id="specific-note">Note ${i + 1}. ${data.note[i].noteText}</h5>`
          );
        }
      } else {
      ...
      notesForm.append(
        `<textarea placeholder="Insert Note Here" id="note" name="note" type="text">`
      );
      notesForm.append(
        `<div><button class="btn btn-primary formbtn" type="submit">Submit Note </button></div>`
      );
